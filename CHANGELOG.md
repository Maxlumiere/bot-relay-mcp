...
- **A notice, metadata only.** `relay: N unread for <agent> (highest priority: P), from <senders> (K high). newest arrived <age> ago. Unread until get_messages is called.` No message content is quoted, not even a first line: `additionalContext` is trusted more than a tool result, so sender-chosen words do not belong in it. Sender names outside `[a-z0-9-]` show as `unknown`. The priority is held to the relay's own values (`critical`, `high`, `normal`, `low`) before it is ranked or shown; any other stored or returned value shows as `unknown`. The newest age is taken over **all** pending mail. When the HTTP page is only part of the pending set, the age comes from the read-only reader, or reads `at an unknown time` when that reader cannot open the DB. It is never taken from the page, which lists by priority and can leave out a brand-new normal message behind older high ones.
