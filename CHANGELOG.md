...
- An **explicit `offline` declaration** (`set_status('offline')`, force token rotation) is NOT overridden by liveness — it's a deliberate "unavailable" statement, distinct from aged-into-offline. Such an agent reads `offline` (and `alive=false`) even while its process is up. The `alive` boolean now requires both a fresh confirmation and an active surfaced status, so it stays consistent with `agent_status`.
