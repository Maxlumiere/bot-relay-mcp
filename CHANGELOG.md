...
- Bug discovery: 2026-05-21 by the maintainer + Victra during v2.7.2 dispatch attempts
...
The v2.8 dashboard-daemon-precursor arc was mid-flight on `hotfix/v2.8-dashboard-state-machine` when these advisories landed in the npm advisory DB (between v2.8 R1 push 2026-05-26 and R2 push 2026-06-02). Per the maintainer's 2026-06-02 lock: ship the security hotfix as a standalone arc off `main` so npm users get the CVE patch immediately, then rebase v2.8 R2 on top of clean main — codex audits each arc on narrow scope rather than juggling a wider mixed scope.
...
- Option A (drop Node 18) locked by the maintainer 2026-06-02 (msg 431b91fc) after scope-expansion ask msg `f701ef85`.
...
the maintainer locked "fix all in one bundled dispatch" on 2026-05-13. PR opened against `main` post-v2.7.0-merge; no v2.7.0 tag was created and no npm publish ran, so v2.7.1 is the first public npm artifact in the v2.7 series.
...
the maintainer's call: **Option A** — make the HTTP daemon support stateful sessions alongside the existing stateless POST. Aligns with v2.3 federation direction (which needs stateful anyway), works for every HTTP MCP client (Cursor, Cline, future tooling), additive (existing stateless POST clients unchanged).
...
- **STOP-AND-SURFACE on architectural forks** — surfaced the Option A/B/C choice with explicit recommendation + reasoning before coding. the maintainer picked A. R0 violated this once (got lucky on subscription state — MCP spec mandated per-session); R1 didn't repeat the violation.

### Hall of Fame

- **the maintainer** — original directive 2026-04-24 to bundle for velocity then split free/paid cleanly. The strict-line architecture is his call. Plus the R1 Option A call: "yeah of course, option a." Read the trade-offs and went straight for the foundation-aligned choice.
...
- **the maintainer** — pushed back ("if it's not user friendly then it's a bug we need to fix") that elevated this from a one-off workaround to a proper architectural fix.
...
Operator-facing friction → product feature. Pre-v2.4.1 the maintainer had to drop to `sqlite3 ~/.bot-relay/relay.db` to answer "which agent has mail piling up?" — the dashboard showed the 20 most-recent messages flat but no per-agent rollup. v2.4.1 closes that gap.
...
- **the maintainer** — the "find bugs at scale without slowing speed" directive after the v2.2.1 get_messages-drops-pending incident that drove Part A. Also "stack as much as possible in one go" — that's what Part A + B + C together deliver.
...
- **the maintainer (public-facing CI failure, 2026-04-22):** caught the red badge + correctly escalated.
...
- **Operator (the maintainer, 2026-04-22):** surfaced the abandoned-agents UX pain after v2.2.1 ship filled the dashboard with retired spawns (B3), and requested the per-human identity cookie so the shared-daemon audit log could tell humans apart (A2).
...
- **Operator surfacing (the maintainer, 2026-04-21 evening):** CLI-args-silently-ignored (B1), `since` filter hiding pending (B4), daemon non-TTY silent exit (B3) — all caught during v2.2.0 ship ceremony.
...
- **external reviewer** — underlying class of TOCTOU vulnerability flagged in the v2.1.7 review that this release closes.
...
### HIGH — `/mcp` Host-header check (external reviewer)
...
### MEDIUM — SameSite cookie + CSRF double-submit infra (external reviewer)
...
### MEDIUM — Per-IP HTTP rate + concurrent cap (external reviewer)
...
- **Audit preview retention (external reviewer):** `audit_log.params_summary` retains 40-char plaintext previews for the audit retention window. Documented in SECURITY.md § "Known residual behavior" with operator-side sanitization guidance; server-side regex scrubbing tracked as v2.1.8 candidate.
...
- **external reviewer** — LAN-deployment security review (original four findings).
...
**This is the flagship v2 release.** Everything works out of the box. Install, register, use — nothing else to configure, no conventions to remember. The guiding principle, from the maintainer: if a user needs to remember a convention to avoid failure, that is a relay bug.
