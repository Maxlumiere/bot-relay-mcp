...
- **A notice, metadata only.** `relay: N unread for <agent> (highest priority: P), from <senders> (K high). newest arrived <age> ago. Unread until get_messages is called.` No message content is quoted, not even a first line: `additionalContext` is trusted more than a tool result, so sender-chosen words do not belong in it. Sender names outside `[a-z0-9-]` show as `unknown`. The count, the highest priority and the newest arrival describe **all** pending mail, never just a page. The read-only reader computes them over the full set. An HTTP page that holds every pending message is the full set. A partial page proves nothing about what it left out, so the full-set reader answers, and with no reader both read `unknown`. An unknown top priority is reminded like a high one. The HTTP peek asks for the whole pending set (`since: "all"`), and a page drawn through any window is never taken for the full set. The count is always the relay's `total_pending`, never the length of a page. A response without that field is not trusted, and the read-only reader answers instead. The damper's fingerprint is built from the whole pending set (count, newest arrival and top priority), so any arrival or resolve re-notifies, including one that sorts outside the page. Priorities are held to the relay's own values before they are ranked or shown. The drain, the summary and the hook share one priority ordering, and it ranks any other value last.
