...
- **New `relay_edge` table** with exactly one row: `edge_id`, a canonical lowercase random UUID v4, created once when the database is set up. It is **never** derived from the instance id (an operator-chosen label, not globally unique), the host id or hostname (host identity must not leave the machine), or a path. The ID carries no information. It is an identifier, never a credential: it proves nothing.
- **Immutable, enforced by the database.** `UPDATE`, `DELETE` and a second `INSERT` on `relay_edge` are refused by triggers. The `INSERT` refusal is what closes `INSERT OR REPLACE`, whose implicit delete fires no `DELETE` trigger. A `CHECK` refuses a malformed value, and the value is **validated on every read**: a missing or malformed edge fails loudly and is never stamped or reported.
- **`agent_bindings.edge_id` (NOT NULL) is stamped from `relay_edge` only**, never from tool arguments, env, stdin or the network. Until signed federation ingress exists, a row carrying any other edge is refused by a trigger. Tests feed a foreign edge through the write object, `RELAY_EDGE_ID` and the SessionStart payload, and the local edge is stamped every time.
- **Every key starts with `edge_id`:** the anchor index, the one-current-binding-per-window unique index, the name index and the conversation index. The anchor lookup, the fleet listing and the end-of-session write are all scoped to the local edge. A new pinned-parser drift guard (`tests/adr-0043-edge-scope-guard.test.ts`) fails on any `agent_bindings` lookup by name, anchor or conversation without `edge_id`, and on any index that does not lead with it. It is checked against forms absent from the repo, e.g. a `+`-split query, a template, a join, and an edge that appears only in the projection.
- **Still schema v25.** v25 has never been released, so a database already holding the pre-edge v25 table is rebuilt in place on open (no v26): the table is copied with every row stamped with the local edge, and the indexes are rebuilt edge-first. `relay bind` and `relay fleet` never migrate. On a database a pre-edge build opened, they refuse with `schema not migrated … pre-edge` rather than failing mid-write.
- **`whoami` and `health_check` report `edge_id`**, so "which store am I talking to?" can be checked. Two clients reporting different `edge_id`s are on two databases.
- `relay_edge` joins the tables the sanctioned-mutation guard allows only `src/db.ts` to write.
- `docs/federation-envelope-v1.md`: `origin_edge` / `target_edge` are now defined as the relay's `relay_edge.edge_id`. A hub is a relay too, so its identity is its own `relay_edge` row.
...
- **One current binding per window anchor is enforced by the DATABASE**, not promised by the writer. A partial unique index over `(edge_id, host_id, window_pid, window_pid_start) WHERE superseded_at IS NULL` (edge-led since edge identity, below) refuses a second current row; the writer's read-decide-write now runs inside one transaction and absorbs the resulting contention (bounded retries with backoff and jitter, inside the published 4200 ms ceiling stated in the next bullet) so a losing process re-reads and refreshes or supersedes instead of surfacing a raw `SQLITE_CONSTRAINT`/`database is locked` to a SessionStart hook. Found in audit round 1: the deciding read previously sat *outside* the transaction, and SQLite serialises individual writes, not a read-then-write decision spread across autocommit statements — so concurrent binds could each observe "no current row" and each insert. **Measured: eight concurrent processes against the old code left 8 current rows; against the new code, exactly 1, with exactly one `created` and no child erroring.** The invariant now lives where two processes cannot both be wrong.
- **The bind retry budget is bounded, and the bound is stated exactly rather than flatteringly.** It is **best-effort budgeting with a bounded per-statement envelope** — *not* a strict whole-operation deadline. The deadline is checked *between* attempts and `busy_timeout` is set per *attempt*, so a statement can consume its own bounded wait after the last check and one attempt may hold several. The honest ceiling is therefore `BUDGET + (max locking statements × busy cap) = 3000 + 3×400 = **4200 ms**`, where 3 is the longest write path (supersede `UPDATE` + `INSERT` + `COMMIT`; the `SELECT` does not block under WAL) — comfortably inside the measured 10 s SessionStart hook timeout, which the health curl, register curl, sqlite reads and wake-coverage subprocess also share. In practice it is lower, because the per-attempt cap is drawn from what remains and shrinks as the deadline nears. **An honest bound with stated slack beats a stricter-sounding one that is still approximate:** threading the deadline through every statement would still be approximate, would cost a pragma per write, and is a no-op on the wasm driver. Exhaustion is loud and fast, naming the elapsed time; it never hangs a session start. The contention classifier was also narrowed: a `UNIQUE` violation counts as contention only when it names the edge and all three anchor columns (measured text — SQLite names *columns*, not the index, so matching the index name would never have fired), and every other error now throws on the **first** failure instead of being retried to exhaustion and relabelled as contention. Found in audit round 2: the previous version bounded its *attempts* and its *own* sleeps (~115 ms) and documented that as the worst case, while a second, uncounted clock ran inside the driver.
- **A correction to that finding, recorded because the fix should not be credited with more than it does.** The feared ~25–30 s ceiling is **not** reachable through the writer. `busy_timeout` governs *acquiring* a lock, not resolving a *snapshot conflict*: a deferred transaction that has already read — even a read matching nothing — cannot upgrade to a write and is refused immediately. Measured under WAL: `read-then-write` → `SQLITE_BUSY` in **0 ms**, `write-first` → **5371 ms**. Since the attempt body reads unconditionally before every write path, in-loop retries fast-fail rather than each burning a busy wait. The genuine 5 s-per-call exposure was on the CLI handle's *out-of-loop* statements, which carried `busy_timeout = 5000` and now carry 1000. The deadline is still correct and still worth having — it bounds the operation rather than an estimate of it — but the arithmetic behind the original finding was sound and *mislocated*.
