...
- **Security**: no escaping relaxation anywhere. New adversarial coverage — codex-launcher path escapes hostile name/cwd (defense-in-depth), and the real-bash suite rejects launcher path-escape / metachars / non-executable / traversal while still enforcing name/cwd hardening on the codex path. Back-compat asserted byte-identical across the env matrix.
