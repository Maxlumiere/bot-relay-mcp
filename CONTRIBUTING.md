...
5. **When you bound something, enumerate every clock that can consume the budget — then assert the TOTAL, not your own contribution to it.** A bound on the part you wrote is not a bound on the operation. (Learned the hard way, the same day as the two above: a bind retry was "bounded" at 6 attempts with ~115ms of JavaScript sleeps, and the comment said so — while the handle carried `busy_timeout = 5000`, a second clock inside the driver that the documented figure never counted. **And then the first correction was wrong too, which is the more useful half.** The obvious arithmetic — 6 attempts × 5s, so a ceiling near 25–30s — was sound *and mislocated*: by §4 that writer reads before every write, so its in-loop retries fast-fail on a snapshot conflict and cannot incur the wait at all. The genuine exposure was the *out-of-loop* single statements, each of which reaches its write in a fresh autocommit transaction. **Neither the original claim nor its first correction survived measurement**; only probing each shape settled it. So: enumerate the clocks, then measure which ones actually run — an estimate of a clock you have not observed is a guess wearing a number. Publish the total as arithmetic a reader can re-derive (`BUDGET + max_locking_statements × cap`), and say plainly when it is best-effort rather than strict. Note the shape of the original error: the *attempt count* was bounded, the *sleep* was bounded, and the *operation* was not.)
