 ...
 *
 * This pins the emitted schema of all tools against a committed golden. Any diff
 * fails LOUD. Do NOT reflexively re-bless the golden: a diff here is a contract
 * change that a client sees. If it is intentional, regenerate deliberately
 * (UPDATE_TOOL_SCHEMA_GOLDEN=1) and review the diff as a client-facing change.
 *
...
describe("tool inputSchema contract — emitted schemas match the golden (external surface)", () => {
  ...
  it.each(Object.keys(golden))("tool '%s' inputSchema is byte-identical to the golden", (name) => {
