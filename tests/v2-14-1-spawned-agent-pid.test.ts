...
describe("v2.14.1 — hook agent_pid capture + spawn-window", () => {
  ...
  it("(B) a populated LIVE row held by a LIVE concurrent terminal → hook SKIPS (agent_pid untouched)", async () => {
    ...
    // v2.23.0: "live" must be a REAL live foreign process (a concurrent
    // terminal's shell), not a fake [999999]. The SessionStart LIVE gate now
    // skips only when the stored chain carries a live pid OUTSIDE this hook's
    // own tree — a dead sentinel is (correctly) treated as a stale relaunch and
    // re-registered. This sleep is not an ancestor of the spawned hook.
