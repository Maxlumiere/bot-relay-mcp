...
describe("LLM adapters", () => {
  it("claude: word (no newline) → delay → SEPARATE sendText(CR)", async () => {
    ...
    expect(ops).toEqual([`sendText:"inbox [tether]":false`, `delay:150`, `sendText:"\\r":false`]);
  });

  it("claude: the submit is a DISTINCT event AFTER the delay (never embedded in the text)", async () => {
    const { ctx, ops } = fakeCtx();
    await claudeAdapter.wake(ctx);
    expect(ops[0]).toBe(`sendText:"inbox [tether]":false`); // the word, never with a newline
  ...
  it("claude: {claude:…} tuning threads submit mechanics (method/delay/key)", async () => {
    ...
    expect(ops).toEqual([`sendText:"inbox [tether]":false`, `delay:200`, `seq:"\\n"`]);
  });
  ...
  it("codex default: word (no newline) → delay → SEPARATE sendText(CR)", async () => {
    ...
    expect(ops).toEqual([
      `sendText:${JSON.stringify(machineWakeText(DEFAULT_CODEX_WAKE_TEXT))}:false`,
      `delay:150`,
      `seq:"\\r"`,
    ]);
  });
  ...
  it("codex: the submit is a DISTINCT event AFTER the delay (not embedded)", async () => {
    const { ctx, ops } = fakeCtx();
    await codexAdapter.wake(ctx);
    expect(ops[0]).toBe(`sendText:${JSON.stringify(machineWakeText(DEFAULT_CODEX_WAKE_TEXT))}:false`); // instruction, never with a newline
  ...
  it("codex sendSequence method: focus-then-sequence submit after the delay", async () => {
    ...
    expect(ops).toEqual([
      `sendText:"ping-off [tether]":false`,
      `delay:150`,
      `seq:"\\r"`,
    ]);
  });
  ...
  it("codex: submitDelayMs=0 skips the delay", async () => {
    ...
    expect(ops).toEqual([`sendText:"ping-off [tether]":false`, `sendText:"\\r":false`]);
  });

  it("codex: wakeText is injected unaltered (the templated INSTRUCTION), with the machine marker appended", async () => {
    const { ctx, ops } = fakeCtx();
    ...
    expect(ops[0]).toBe(`sendText:${JSON.stringify(machineWakeText(instruction))}:false`);
  });
  ...
  it("adapterFor: codex id → codex adapter with options threaded", async () => {
    ...
    await a.wake(ctx);
    expect(ops).toEqual([`sendText:"ping-off [tether]":false`, `delay:150`, `seq:"\\n"`]);
  });
...
describe("profile isolation — codex options must not reshape the claude profile", () => {
  ...
  it("adapterFor('codex', {codex:…}) DOES still honour its own tuning (positive control)", () => {
    ...
    return adapter.wake(ctx as never).then(() => {
      // The profile's OWN tuning still reaches the wire — that is what this
      // positive control protects. The marker is appended by the injector and
      // is orthogonal to which profile's text was chosen.
      expect(sent[0]).toBe(machineWakeText("CUSTOM-CODEX-TEXT"));
      expect(sent).toContain("<CR-sequence>");
    });
  });
});

// ═══ LINK 3 — machine-wake marker, against the REAL adapter submit path ═══

describe("machine-wake marker", () => {
  it("HARM: an injected wake is distinguishable from a human typing the wake word", () => {
    // The defect: claude's wake word is the literal string `inbox`, byte-identical
    // to what the operator types. No receiver could tell the two apart, so no
    // agent could honestly report its own wake state.
    expect(machineWakeText("inbox")).toContain(MACHINE_WAKE_MARKER);
    expect(machineWakeText("inbox")).not.toBe("inbox");
  });

  it("the marker rides the REAL submit sequence, not a mock of it", async () => {
    // Condition of approval: verified against the actual adapter. The
    // 2026-07-24 defect was a types-but-never-submits wake, and a mock of the
    // sequence passes that happily.
    const { ctx, ops } = fakeCtx();
    await claudeAdapter.wake(ctx);
    expect(ops[0]).toBe(`sendText:"inbox [tether]":false`);
    expect(ops[1]!.startsWith("delay:")).toBe(true);
    expect(ops[2]).toBe(`sendText:"\\r":false`);
    expect(ops).toHaveLength(3);
  });

  it("INNOCENT TWIN: the CLI still takes a turn — submit stays a SEPARATE event", async () => {
    // The marker must not merge the type and the submit, must not add a newline
    // to the typed chunk, and must not change the submit key. All three are the
    // v0.7.0 / 2026-07-24 regressions.
    for (const adapter of [claudeAdapter, codexAdapter]) {
      const { ctx, ops } = fakeCtx();
      await adapter.wake(ctx);
      expect(ops[0]!.endsWith(":false"), "marker added a newline to the typed chunk").toBe(true);
      expect(ops.some((o) => o.startsWith("delay:")), "paste-settle delay lost").toBe(true);
      expect(ops.length, "type and submit collapsed into one event").toBeGreaterThanOrEqual(3);
    }
  });

  it("the leading token the agent acts on is unchanged — marker is trailing", () => {
    // Why the CLI still behaves: the instruction comes first, the marker after.
    expect(machineWakeText("inbox").startsWith("inbox")).toBe(true);
    expect(machineWakeText(DEFAULT_CODEX_WAKE_TEXT).startsWith(DEFAULT_CODEX_WAKE_TEXT)).toBe(true);
  });

  it("the marker is ASCII — an encoding a TUI mangles is a wake that never submits", () => {
    // eslint-disable-next-line no-control-regex
    expect(/^[\x20-\x7e]+$/.test(MACHINE_WAKE_MARKER)).toBe(true);
  });

  it("DOCUMENTS THE ASYMMETRY: absence proves nothing", () => {
    // Presence ⇒ this extension injected it. Absence ⇒ NOT a proof of a human,
    // because this extension is not proven to be the only thing that can type
    // into a terminal. The doc comment must keep saying so, or a future reader
    // completes the inference themselves and manufactures a false certainty.
    const src = readFileSync(new URL("./llm-adapter.ts", import.meta.url), "utf8");
    expect(src).toMatch(/ABSENT.*NOT SOUND|PROVES NOTHING/s);
  });
});
