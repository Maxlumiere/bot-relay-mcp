...
This is a manual checklist for the maintainer to run when ready to publish the extension to the VSCode Marketplace. The extension does NOT auto-publish from CI — marketplace publication is gated on the Marketplace publisher account, which only the maintainer holds.
...
- Watch the marketplace stats for the first 2 weeks — that's the validation signal the maintainer named at dispatch (operators using it without falling back to typing `inbox`).
