...
#
# WIRED DIRECT (#206 step model): invoked as `step "…" node <guard> <target>` — NOT via a
# wrapper function — so `--list-steps` records the node-script position and
...
step "sanctioned-mutation guard (no raw agents/agent_capabilities/agent_bindings/relay_edge writes outside src/db.ts)" node "$PROJECT_ROOT/scripts/sanctioned-mutation-guard.mjs" "$PROJECT_ROOT/src" || exit 1
