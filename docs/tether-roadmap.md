...
3. **Documentation** — this file, README pointer, CHANGELOG entry, and `extensions/vscode/PUBLISH.md` for the marketplace publish steps the maintainer runs when ready.
...
The Phase 2 brief gets written **only after** Phase 1 ships and validates that operators actually use it. Validation signal the maintainer named at dispatch time: he uses Tether himself for at least 2 weeks of dispatch arcs without falling back to typing `inbox` by hand.

## Pricing (TBD)

Strategic decision deferred to the maintainer — pricing model, free-tier limits, paid-tier features mix. This file gets updated when that lands; until then, this section is intentionally a placeholder so the read order is "free / paid line first, pricing later."
