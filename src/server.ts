...
export function createServer(): Server {
  ...
  server.setRequestHandler(SubscribeRequestSchema, async (request) => {
    const { uri } = request.params;
    // v2.7.0 — debug-level RPC-arrival trace. The semantic "subscriber
    // now registered" event surfaces at info via
    // src/mcp-subscriptions.ts `subscribe added` line, which fires on
    // the very next statement. Both info+debug here was redundant.
    log.debug(`[broadcast-trace] resources/subscribe RPC arrived uri=${uri}`);
    subscribeResource(uri, server);
    return {};
  });
  server.setRequestHandler(UnsubscribeRequestSchema, async (request) => {
    const { uri } = request.params;
    log.debug(`[broadcast-trace] resources/unsubscribe RPC arrived uri=${uri}`);
  ...
  const ALL_TOOLS_DEFINITION = [
      ...
      },
      {
        ...
        description:
          ...
          "Behavior: returns messages addressed to you, ordered by priority then `created_at` newest-first. By default `status='pending'` returns un-read messages and atomically marks them read for THIS session (sessions are per-`session_id`; a fresh terminal re-sees previously-read messages, v2.0 final fix). Optional `since` (`'1h' | '24h' | '7d' | ISO | 'all'`); its DEFAULT DEPENDS ON STATUS: `status='pending'` defaults to `'all'` (the action queue is never windowed unless you ask), history reads (`all`/`history`/`read`/`resolved`) default to `'24h'`. A pending read ALWAYS returns UNDELIVERED (never-drained) mail regardless of `since` (#198; 3.0.1). For a pending read `total_pending` is always the full, unwindowed queue, and when your `since` hid pending mail the response carries `hidden_by_since: N`. `peek=true` (v2.2.2) suppresses the read-MARK — the returned messages are not marked read for your session — but it is NOT side-effect-free: like any first view it stamps the observation cursor (`seq`). (Post-3.0.1 that observation no longer changes what a later drain returns.)\n\n" +
          "Returns: `{ messages: MessageRecord[], count, has_more, total_pending | total, hidden_by_since?, agent, filter, since, since_bound }`. `since` is the window actually applied (the status default when you gave none); `since_bound` is the ISO timestamp the relay filtered by (after resolving duration shorthands or `'session_start'`).\n\n" +
      ...
      },
      {
        ...
        description:
          ...
          "Behavior: returns the messages YOU sent with `disposition` in ('ask','obligation'), each with its sender-visible lifecycle `state` (undrained / drained-unresolved / resolved — a DELIVERY-PATH state: `undrained` means not drained through the consuming get_messages path, which is NOT the same as unread/unseen, since peek/CLI/direct-DB reads never stamp it) and a REPORT-ONLY `overdue` flag computed at query time — it NEVER mutates a message (report-first, never auto-resolve). LOG messages are excluded (LOG never goes overdue). `overdue` = still-unresolved AND past its bound: an obligation past its `deadline`, else an ask/deadline-less-obligation past `created_at + RELAY_OVERDUE_SECONDS` (default 24h, tunable). `include_resolved=false` (default) returns only the outstanding set; true adds resolved rows for the full sender view. Auth: agent token; sender-scoped (you only see mail you sent).\n\n" +
      ...
      },
      {
        ...
        description:
          ...
          "Behavior: sets `resolved_at=now()` for the eligible ids, in one transaction. Does NOT mark messages read (orthogonal plane) and does NOT delete them — they remain in `status='all'`/`'history'`/`'resolved'`. Idempotent: re-resolving, unknown ids, or ineligible ids are silently skipped (reflected in the returned counts). Scope: you can resolve mail addressed to YOU (recipient-resolve), OR an obligation/message YOU sent to a recipient that is NOT a registered agent (sender-resolve) — a human (e.g. a person you owe an answer) has no token, so without this such mail could never be cleared and the pending-on-a-human lane would be write-only. You can NEVER resolve mail addressed to a real agent — that stays the recipient's to clear. The dispatcher binds your token to `agent_name`, so there is no impersonation path. Edge: the non-agent check is evaluated at resolve time, so if a human recipient later REGISTERS as an agent the sender can no longer resolve it — `note` names that case so a legitimate zero doesn't read as a failure.\n\n" +
          "Returns: `{ success: true, agent, resolved_ids: string[], resolved_count, requested_count, blocked_by_recipient_registration: {id,to_agent}[], note }`. `resolved_count` < `requested_count` when some ids were already resolved, unknown, not yours, or (per `blocked_by_recipient_registration`) sent by you to a recipient that has since become a registered agent.\n\n" +
