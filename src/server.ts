  ...
  const ALL_TOOLS_DEFINITION = [
      ...
      },
      {
        ...
        description:
          ...
          "Behavior: pure read. Returns `{ mailbox_id, epoch, last_seq, total_messages_count, total_unread_count }`. WATCH `total_unread_count` for new-mail detection, it advances on every `send_message`/`broadcast` to this agent. `last_seq` is the OBSERVED axis: it advances when the recipient views mail through ANY `get_messages` call, including `peek=true`, so it says nothing about delivery or reading, and no decision may key on it. `epoch` rotates on backup/restore, a client whose cached epoch no longer matches MUST reset its local `last_seen_seq` to 0 and re-drain. Auth: any agent token.\n\n" +
