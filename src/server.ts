  ...
  const ALL_TOOLS_DEFINITION = [
      ...
      },
      {
        ...
        description:
          ...
          "Behavior: returns messages addressed to you, ordered by priority then `created_at` newest-first. By default `status='pending'` returns un-read messages and atomically marks them read for THIS session (sessions are per-`session_id`; a fresh terminal re-sees previously-read messages, v2.0 final fix). Optional `since` (`'1h' | '24h' | '7d' | ISO | 'all'`) bounds already-OBSERVED history (v2.1.6 default `'24h'`); a pending drain ALWAYS returns never-observed (undelivered) mail regardless of `since` (#198). When `status='pending'` returns 0 with `since<24h`, the response includes a `hint` field nudging toward `since='all'` for older already-seen mail. `peek=true` (v2.2.2) suppresses the read-side-effect entirely.\n\n" +
