  ...
  const ALL_TOOLS_DEFINITION = [
      ...
      },
      {
        ...
        description:
          ...
          "Returns: `{ messages: MessageRecord[], count, agent, filter, since, since_bound, hint?, acked?, resolved_count?, warning? }`. `since_bound` is the ISO timestamp the relay actually filtered by (after resolving duration shorthands or `'session_start'`). With `ack=true` on a pending drain, `acked` and `resolved_count` report what the database actually changed, never what was asked. If your agent has no current session (`session_id` NULL, e.g. after a token rotation), the drain still delivers and records the agent-level receipts (read_at, last_drain_at, resolves) and returns `warning: { code: 'session_unbound', message }`: unresolved mail comes back on the next drain until you re-register or resolve it.\n\n" +
